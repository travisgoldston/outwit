      ...
      <Reveal className="mx-auto max-w-6xl rounded-3xl border border-slate-200 bg-white p-8 sm:p-10">
        ...
        <div className="mt-16 grid gap-8 sm:grid-cols-3">
          {solutions.map((solution, idx) => (
            <Reveal key={solution.title} delayMs={80 * idx}>
              <Link
                href={solution.href}
                className="group block rounded-2xl border border-slate-200 bg-white p-8 text-left shadow-sm transition-all duration-300 hover:border-neon-orange/60 hover:shadow-sm"
              >
